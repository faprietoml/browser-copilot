...
## Reasoning 
The executor agent is of the Zero-shot type, and at each step the agent follows the pattern Thought → Action → Action Input → Observation, repeating until the final response is generated.

The steps to answer a question are:
1. Agent is initialized.
2. An async callback is created to capture the tokens generated by the question.
3. The async iterator is created to capture the result of the task.
4. The async task is created to execute the routine.
5. The task is started.
6. After the task executes, it iterates asynchronously through the results.

The task can be canceled by sending a request to [cancel](http://localhost:8000/sessions/${SESSION_ID}/cancel) endpoint. The iterator is interrupted and the task is canceled.
...
### Stop current question processing

```bash
curl -X POST -H "Content-Type: application/json" --data '{}' http://localhost:8000/sessions/${SESSION_ID}/cancel
```