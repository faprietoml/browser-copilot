...
const onAgentResponse = (text: string, complete: boolean) => {
  ...
  try {
    ...
    switch (msgType) {
      case "token":
        lastMessage.reasoning += msgValue;
        break;

      case "event":
        switch (msgValue) {
          case "cancellation":
            // TODO: mejorar estilos de info/success para reemplazar el toast.error
            toast.error({
              component: ToastMessage,
              props: { message: "Cancellation completed" }
            }, { icon: AlertCircleFilledIcon });

            lastMessage.text += "Action cancelled";
            break;

          default:
            console.error(`Event not implemented: ${msg}`);
        }
        break;

      case "error":
        console.error("Received error: ", msgValue);
        lastMessage.text += msgValue;
        break;

      default:
        console.error(`Type not implemented: ${msg}`);
    }
  } catch (err) {
...
<template>
  <div
    ...
    <CopilotChat v-if="agent" :messages="messages" :agent-id="agent.manifest.id" :agent-name="agent.manifest.name"
                 :agent-logo="agent.logo" :agent-capabilities="agent.manifest.capabilities || []"
